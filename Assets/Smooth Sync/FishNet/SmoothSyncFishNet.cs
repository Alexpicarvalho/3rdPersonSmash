...
namespace Smooth
{
    ...
    /// </remarks>
    public class SmoothSyncFishNet : NetworkBehaviour
	{
        ...
        /// </remarks>
        float maxLocalTime = Mathf.Pow(2, maxTimePower);
        ...
        /// </remarks>
        float minTimePrecision = Mathf.Pow(2, maxTimePower - 24);
        ...
        public Channel networkChannel = Channel.Unreliable;
        ...
        /// </remarks>
        public delegate bool validateStateDelegate(StateFishNet receivedState, StateFishNet latestVerifiedState);
        ...
        /// </remarks>
        public static bool validateState(StateFishNet latestReceivedState, StateFishNet latestValidatedState)
        {
            return true;
        }
        ...
        /// </remarks>
        StateFishNet latestValidatedState;
        bool hasValidatedState = false;
        ...
        [NonSerialized]
        public StateFishNet[] stateBuffer;
        ...
        StateFishNet latestEndStateUsed;
        ...
        public bool hasAuthorityOrUnownedOnServer {
            get {
                return netID.IsOwner || (NetworkManager.IsServer && netID.Owner == FishNet.Managing.NetworkManager.EmptyConnection);
            }
        }

        public bool hasControl {
            get {
                if ((transformSource == TransformSource.Owner && hasAuthorityOrUnownedOnServer) || (transformSource == TransformSource.Server && NetworkManager.IsServer))
                {
                    return true;
                }
                else
        ...
        public void Awake()
        {
            ...
            stateBuffer = new StateFishNet[Mathf.Max(calculatedStateBufferSize, 30)];

            realObjectToSync = this.gameObject;

            netID = GetComponent<NetworkObject>();
            ...
            targetTempState = new StateFishNet();
            sendingTempState = new StateFishNet();
        }
        ...
        void sendState()
        {
            ...
            if (!hasControl || (!NetworkManager.IsServer && !NetworkManager.ClientManager.Started) || sendRate == 0) return;
            ...
            if (localTime - lastTimeStateWasSent < 1/sendRate && !forceStateSend) return;
            ...
            else
            {
                ...
                RPCHandleSyncFromOwnerToServer(sendingTempState, networkChannel);
            }
        }
        ...
        {
            if (hasAuthorityOrUnownedOnServer && !hadAuthorityLastFrame && stateCount > 0)
            {
                if (hasRigidbody)
                {
        ...
        void interpolate(float interpolationTime)
        {
            ...
            StateFishNet end = stateBuffer[Mathf.Max(stateIndex - 1, 0)];
            // The starting playback State.
            StateFishNet start = stateBuffer[stateIndex];
        ...
        void shouldTeleport(StateFishNet start, ref StateFishNet end, float interpolationTime, ref float t)
        {
            ...
            for (int i = 0; i < stateCount; i++)
            {
                if (stateBuffer[i].ownerTimestamp == latestEndStateUsed.ownerTimestamp && latestEndStateUsed.ownerTimestamp != end.ownerTimestamp && latestEndStateUsed.ownerTimestamp != start.ownerTimestamp)
                {
                    for (int j = i - 1; j >= 0; j--)
                    {
                        ...
                        if (stateBuffer[j].ownerTimestamp == start.ownerTimestamp) break;
                    }
                    break;
                }
            }
        ...
        public void addState(StateFishNet state)
        {
            if (stateCount > 1)
            {
        ...
        /// <summary>Teleport the object, the transform will not be interpolated on non-owners.</summary>
        /// <remarks>
        ...
        /// <summary>Echoes a teleport State from the host to all clients.</summary>
        [ObserversRpc]
        public void CmdTeleport(Vector3 position, Vector3 rotation, Vector3 scale, float tempOwnerTime)
        {
            ...
            StateFishNet teleportState = new StateFishNet();
        ...
        /// <summary>Receive teleport State on clients and add to State array.</summary>
        [ObserversRpc]
        public void RpcTeleport(Vector3 position, Vector3 rotation, Vector3 scale, float tempOwnerTime)
        {
            ...
            if (hasAuthorityOrUnownedOnServer || NetworkManager.IsServer) return;
        ...
        /// <summary>Add the teleport State at the correct place in the State buffer.</summary>
        void addTeleportState(StateFishNet teleportState)
        {
        ...
        /// <summary>Forces the State to be sent on owned objects the next time it goes through FixedUpdate().</summary>
        /// <remarks>
        ...
        /// <summary>Check if position has changed enough.</summary>
        /// <remarks>
        ...
        /// <summary>Check if rotation has changed enough.</summary>
        /// <remarks>
        ...
        /// <summary>Check if scale has changed enough.</summary>
        /// <remarks>
        ...
        /// <summary>Check if velocity has changed enough.</summary>
        /// <remarks>
        ...
        /// <summary>Check if angular velocity has changed enough.</summary>
        /// <remarks>
...
#region Sync Properties
        ...
        void SendStateToNonOwners(StateFishNet state)
        {
            foreach (NetworkConnection conn in netID.Observers)
            {
                // Can't send to a null connection
                if (conn == null) continue;
                // Don't send owner's state back to them
                if (transformSource == TransformSource.Owner && conn == netID.Owner) continue;
                // Don't send to the local client, it is either the owner or already applied the state when it was received from the owner
                if (conn.IsLocalClient) continue;

                // Send the message, this calls HandleSyncFromServerToNonOwners on the receiving clients.
                RPCHandleSyncFromServerToNonOwners(state, networkChannel, conn);
            }
        }
        ...
        /// </remarks>
        [ObserversRpc]
        void RPCHandleSyncFromServerToNonOwners(StateFishNet state, Channel channel = Channel.Reliable, NetworkConnection connection = null)
        {
            if (state.smoothSync != null && !state.smoothSync.hasControl)
            {
                state.smoothSync.addState(state);
                state.smoothSync.checkIfOwnerHasChanged(state);
            }
        }
        ...
        /// </remarks>
        [ServerRpc]
        void RPCHandleSyncFromOwnerToServer(StateFishNet state, Channel channel = Channel.Reliable, NetworkConnection connection = null)
        {
            if (state.smoothSync == null || state.smoothSync.netID.Owner != connection) return;
            ...
            if (!state.smoothSync.hasValidatedState || state.smoothSync.validateStateMethod(state, state.smoothSync.latestValidatedState))
            {
                state.smoothSync.hasValidatedState = true;
                state.smoothSync.latestValidatedState = state;
                state.smoothSync.latestValidatedState.receivedOnServerTimestamp = state.smoothSync.localTime;
                state.smoothSync.SendStateToNonOwners(state);
                state.smoothSync.addState(state);
                state.smoothSync.checkIfOwnerHasChanged(state);
            }
        }
        ...
        public void checkIfOwnerHasChanged(StateFishNet newState)
        {
            ...
            {                
                ...
                StateFishNet simulatedState = new StateFishNet();
        ...
        void adjustOwnerTime() 
        {
            ...
            if ((stateBuffer[0].atPositionalRest && stateBuffer[0].atRotationalRest)) return;
