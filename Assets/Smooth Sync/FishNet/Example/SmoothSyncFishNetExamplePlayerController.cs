...
/// </remarks>
public class SmoothSyncFishNetExamplePlayerController : NetworkBehaviour
{
    Rigidbody rb;
    Rigidbody2D rb2D;
    SmoothSyncFishNet smoothSync;

    /// <summary>The speed to move per second when there is no rigidbody component on the player</summary>
    public float transformMovementSpeed = 30.0f;

    /// <summary>The force to add on key up when there is a rigidbody component on the player</summary>
    public float rigidbodyMovementForce = 500;

    public GameObject childObjectToControl;

    /// <summary>Cache a reference to the rigidbody component and Smooth Sync scripts.</summary>
	void Start ()
    {
        rb = GetComponent<Rigidbody>();
        rb2D = GetComponent<Rigidbody2D>();
        smoothSync = GetComponent<SmoothSyncFishNet>();
    ...
    /// <summary>Simple movement.</summary>
    /// <remarks>
    ...
    void Update ()
    {
        ...
        if (!(IsOwner || IsServer && Owner == NetworkManager.EmptyConnection)) return;
    ...
    /// <summary>Custom validation method.</summary>
    /// <remarks>
    ...
    /// </remarks>
    public static bool validateStateOfPlayer(StateFishNet latestReceivedState, StateFishNet latestValidatedState)
    {
